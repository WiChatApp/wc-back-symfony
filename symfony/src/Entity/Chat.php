...
class Chat extends AbstractEntity
{
	...
	#[ORM\OneToMany(targetEntity: Assignation::class, mappedBy: 'chat', orphanRemoval: true)]
	...
	public function removeAssignation(Assignation $assignation): static
	{
		if ($this->assignations->removeElement($assignation)) {
			// set the owning side to null (unless already changed)
			if ($assignation->getChat() === $this) {
				$assignation->setChat(null);
			}
		}
